...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM UserComplaintRoles WHERE UserId = @userId AND RoleId = @roleId)
    BEGIN
        INSERT INTO UserComplaintRoles (UserId, RoleId, IsPrimary, IsActive, EffectiveFrom)
    ...
    BEGIN
        ...
        SET IsActive = 1, IsPrimary = 1, EffectiveFrom = GETUTCDATE()
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM UserComplaintRoles WHERE UserId = @handlerUserId AND RoleId = @handlerRoleId)
    BEGIN
        INSERT INTO UserComplaintRoles (UserId, RoleId, IsPrimary, IsActive, EffectiveFrom)
    ...
    BEGIN
        UPDATE UserComplaintRoles
        SET IsActive = 1, IsPrimary = 1, EffectiveFrom = GETUTCDATE()
        WHERE UserId = @handlerUserId AND RoleId = @handlerRoleId;
...
JOIN ComplaintRoles r ON ucr.RoleId = r.Id
