...
# =============================================
# CATEGORY 1: ORGANIZATION STRUCTURE (24 tests)
# =============================================
Write-Log "============================================="
Write-Log "CATEGORY 1: ORGANIZATION STRUCTURE (24 TESTS)"
Write-Log "============================================="

# Employee Types (6 tests)
Write-Log "--- Employee Types Management (6 tests) ---"

$empTypes = Test-APIEndpoint "Org Structure" "Get All Employee Types" "GET" "/api/employee-types?companyId=$CompanyId" -Headers $authHeaders

$createEmpTypeBody = @{
    name = "Test Employee Type $(Get-Random -Minimum 100000 -Maximum 999999)"
    code = "TEST_EMP_$(Get-Random -Minimum 100000 -Maximum 999999)"
    description = "Test employee type"
    companyId = $CompanyId
    displayOrder = 100
    isActive = $true
} | ConvertTo-Json

$createdEmpType = Test-APIEndpoint "Org Structure" "Create Employee Type" "POST" "/api/employee-types" -Body $createEmpTypeBody -Headers $authHeaders -ExpectedStatuses @(200, 201)

if ($createdEmpType -and $createdEmpType.data) {
    $empTypeId = $createdEmpType.data.id

    $getEmpType = Test-APIEndpoint "Org Structure" "Get Employee Type by ID" "GET" "/api/employee-types/$empTypeId" -Headers $authHeaders

    $updateEmpTypeBody = @{
        name = "Updated Test Employee Type"
        code = $createdEmpType.data.code
    ...
    Test-APIEndpoint "Org Structure" "Update Employee Type" "PUT" "/api/employee-types/$empTypeId" -Body $updateEmpTypeBody -Headers $authHeaders
    Test-APIEndpoint "Org Structure" "Delete Employee Type" "DELETE" "/api/employee-types/$empTypeId" -Headers $authHeaders
}

Test-APIEndpoint "Org Structure" "Validation: Empty Employee Type Name" "POST" "/api/employee-types" -Body (@{ name = ""; code = "TEST"; companyId = $CompanyId } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure
...
# =============================================
# CATEGORY 6: COMMUNICATION SETTINGS (24 tests)
# =============================================
Write-Log "============================================="
Write-Log "CATEGORY 6: COMMUNICATION SETTINGS (24 TESTS)"
Write-Log "============================================="
...
# SMS Gateway Settings (8 tests)
Write-Log "--- SMS Gateway Settings (8 tests) ---"

# 

$smsSettings = Test-APIEndpoint "Communication" "Get All SMS Settings" "GET" "/api/sms-gateway" -Headers $authHeaders

$createSmsSettingBody = @{
    name = "Test SMS Gateway"
    provider = "Twilio"
    accountSid = "test_api_key"
    authToken = "test_api_secret"
...
$createdSmsSetting = Test-APIEndpoint "Communication" "Create SMS Setting" "POST" "/api/sms-gateway" -Body $createSmsSettingBody -Headers $authHeaders -ExpectedStatuses @(200, 201)

if ($createdSmsSetting -and $createdSmsSetting.id) {
    $smsSettingId = $createdSmsSetting.id

    Test-APIEndpoint "Communication" "Get SMS Setting by ID" "GET" "/api/sms-gateway/$smsSettingId" -Headers $authHeaders

    $updateSmsSettingBody = @{
        id = $smsSettingId
        name = "Updated SMS Gateway"
        provider = "Twilio"
        accountSid = "updated_api_key"
        authToken = "updated_api_secret"
        fromNumber = "+0987654321"
        apiUrl = "https://api.updated.com/sms"
        isDefault = $false
        isActive = $true
    } | ConvertTo-Json

    Test-APIEndpoint "Communication" "Update SMS Setting" "PUT" "/api/sms-gateway/$smsSettingId" -Body $updateSmsSettingBody -Headers $authHeaders

    Test-APIEndpoint "Communication" "Delete SMS Setting" "DELETE" "/api/sms-gateway/$smsSettingId" -Headers $authHeaders
}

Test-APIEndpoint "Communication" "Get Inactive SMS Settings" "GET" "/api/sms-gateway?includeInactive=true" -Headers $authHeaders

Test-APIEndpoint "Communication" "Validation: Empty Provider" "POST" "/api/sms-gateway" -Body (@{ name = "Test"; provider = ""; apiKey = "test" } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure

Test-APIEndpoint "Communication" "Validation: Empty API Key" "POST" "/api/sms-gateway" -Body (@{ name = "Test"; provider = "Twilio"; apiKey = "" } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure

# WhatsApp Settings (8 tests)
Write-Log "--- WhatsApp Settings (8 tests) ---"

# 

$whatsappSettings = Test-APIEndpoint "Communication" "Get All WhatsApp Settings" "GET" "/api/whatsapp-settings" -Headers $authHeaders

$createWhatsAppSettingBody = @{
    name = "Test WhatsApp Business"
    provider = "Twilio"
    apiKey = "test_whatsapp_key"
    apiSecret = "test_whatsapp_secret"
...
$createdWhatsAppSetting = Test-APIEndpoint "Communication" "Create WhatsApp Setting" "POST" "/api/whatsapp-settings" -Body $createWhatsAppSettingBody -Headers $authHeaders -ExpectedStatuses @(200, 201)

if ($createdWhatsAppSetting -and $createdWhatsAppSetting.id) {
    $whatsappSettingId = $createdWhatsAppSetting.id

    Test-APIEndpoint "Communication" "Get WhatsApp Setting by ID" "GET" "/api/whatsapp-settings/$whatsappSettingId" -Headers $authHeaders

    $updateWhatsAppSettingBody = @{
        id = $whatsappSettingId
        name = "Updated WhatsApp Business"
        provider = "Twilio"
        apiKey = "updated_whatsapp_key"
        apiSecret = "updated_whatsapp_secret"
        phoneNumberId = "+0987654321"
        businessAccountId = "updated_business_id"
        apiEndpoint = "https://api.whatsapp.updated.com"
        isDefault = $false
        isActive = $true
    } | ConvertTo-Json

    Test-APIEndpoint "Communication" "Update WhatsApp Setting" "PUT" "/api/whatsapp-settings/$whatsappSettingId" -Body $updateWhatsAppSettingBody -Headers $authHeaders

    Test-APIEndpoint "Communication" "Delete WhatsApp Setting" "DELETE" "/api/whatsapp-settings/$whatsappSettingId" -Headers $authHeaders
}

Test-APIEndpoint "Communication" "Get Inactive WhatsApp Settings" "GET" "/api/whatsapp-settings?includeInactive=true" -Headers $authHeaders

Test-APIEndpoint "Communication" "Validation: Empty Phone Number" "POST" "/api/whatsapp-settings" -Body (@{ name = "Test"; provider = "Twilio"; phoneNumberId = "" } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure

Test-APIEndpoint "Communication" "Validation: Empty Business Account" "POST" "/api/whatsapp-settings" -Body (@{ name = "Test"; provider = "Twilio"; businessAccountId = "" } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure
...
# =============================================
# CATEGORY 9: COMPANY & SETTINGS (12 tests)
# =============================================
Write-Log "============================================="
Write-Log "CATEGORY 9: COMPANY & SETTINGS (12 TESTS)"
Write-Log "============================================="
...
$infoSettings = Test-APIEndpoint "Company" "Get Complaint Info Settings" "GET" "/api/complaint-info-settings?companyId=$CompanyId" -Headers $authHeaders

$createInfoSettingBody = @{
    companyId = $CompanyId
    requireBranch = $true
...
Test-APIEndpoint "Company" "Create/Update Info Setting" "PUT" "/api/complaint-info-settings/$CompanyId" -Body $createInfoSettingBody -Headers $authHeaders -ExpectedStatuses @(200, 201, 400)

Test-APIEndpoint "Company" "Get Info Settings by Company" "GET" "/api/complaint-info-settings?companyId=$CompanyId" -Headers $authHeaders

Test-APIEndpoint "Company" "Toggle Anonymous Setting" "PUT" "/api/complaint-info-settings/$CompanyId/anonymous" -Body (@{ allowAnonymous = $false } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(200, 404)

Test-APIEndpoint "Company" "Toggle Attachments Setting" "PUT" "/api/complaint-info-settings/$CompanyId/attachments" -Body (@{ allowAttachments = $false } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(200, 404)

Test-APIEndpoint "Company" "Validation: Invalid Max Attachments" "PUT" "/api/complaint-info-settings/$CompanyId" -Body (@{ companyId = $CompanyId; maxAttachments = -1 } | ConvertTo-Json) -Headers $authHeaders -ExpectedStatuses @(400) -ExpectFailure
